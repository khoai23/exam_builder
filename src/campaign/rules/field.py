...
TERRAIN_BONUSES = { # only have defensive coef & preserve (occupy) penalty for now
    "urban":  (0.0, 4.0), # no defense boost, very heavy occupy penalty, presenting relatively easy-to-take but hard-to-hold
...
class TerrainRule(Rule):
    ...
    def __init__(self, campaign):
        ...
        non_spec_terrain = [t for t in TERRAIN if t not in ("urban", "desert")]
        for *_, p in self.campaign._map:
            if p["score"] >= 9:
                # enforce to be urban 
                p["terrain"] = "urban"
                ...
                # to be the rest 
                p["terrain"] = random.choice(non_spec_terrain)
